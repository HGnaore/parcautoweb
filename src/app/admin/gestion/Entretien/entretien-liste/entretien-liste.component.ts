...
@Component({
  ...
  styleUrls: ['./entretien-liste.component.scss'],
   animations: [
...
export class EntretienListeComponent implements OnInit {
  ...
  Locations: any = [];
  listeLocations: any = [];
  displayedColumns: string[] = ['number','immatriculation', 'dateEntretien', 'montant', 'fournisseurName','objetEntretien', 'action'];
  ...
  listeEntretien() {
    this.entretienService.listeEntretiens().subscribe(reponse => {
  ...
  supprime() {
    this.entretienService.deleteEntretiens(this.IDSuppression).subscribe(reponse => {
